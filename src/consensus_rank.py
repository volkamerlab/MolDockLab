...
def method1_ECR_best(df: pd.DataFrame, weight: float, selected_scores: list, id_column: str) -> pd.DataFrame:
    '''
    A method that calculates the ECR score for each ID in the rescored dataframe and returns the ID for the pose with the best ECR rank.
    Args:
...
def method2_ECR_average(df: pd.DataFrame, weight: float, selected_scores: list, id_column: str) -> pd.DataFrame:
    '''
    A method that calculates the ECR score for each ID in the rescored dataframe and returns the ID with ECR average rank.
    Args:
        df (DataFrame): The dataframe containing the rescored poses
        weight (float): The inclusion threshold for the poses
        selected_scores (list): The list of columns to be used for calculating the ECR
...
def method3_avg_ECR(df: pd.DataFrame, weight: float, selected_scores: list, id_column: str) -> pd.DataFrame:
    '''
    A method that calculates the ECR score for each ID after taking the average of ID's different poses and returns the ID with the ECR rank.
    Args:
        df (DataFrame): The dataframe containing the rescored poses
        weight (float): The inclusion threshold for the poses
        selected_scores (list): The list of columns to be used for calculating the ECR
    ...
    # df[selected_scores] = df[selected_scores].rank(
    #     method='average', ascending=1)
    sigma = weight * len(df)
    # df[selected_scores] = df[selected_scores].rank(
    #     method='average', ascending=1)
    # df[selected_scores] = df[selected_scores].rank(method='min', ascending=True)
    for col in selected_scores:
        # df.loc[:, col] = df.loc[:, col].rank(method='min', ascending=False)
        df.loc[:, col] = df.loc[:, col].rank(method='min', ascending=False)
        df.loc[:, col] = (np.exp(-(df.loc[:, col] / sigma)) / sigma) 
...
def method4_RbR(df: pd.DataFrame, weight: float, selected_scores: list, id_column: str) -> pd.DataFrame:
    '''
...
def method5_RbV(df: pd.DataFrame, weight: float, selected_scores: list, id_column: str) -> pd.DataFrame:
    '''
...
def method6_Zscore_best(df: pd.DataFrame, weight: float, selected_scores: list, id_column: str) -> pd.DataFrame:
    '''
...
def method7_Zscore_avg(df: pd.DataFrame, weight: float, selected_scores: list, id_column: str) -> pd.DataFrame:
    '''
    Calculates the Z-score consensus scores for each row in the given DataFrame,
...
def method8_RbN(df: pd.DataFrame, weight: float, selected_scores: list, id_column: str) -> pd.DataFrame:
    '''
...
def method9_weighted_ECR_best(df: pd.DataFrame, mapped_weights:dict, selected_scores: list, id_column:str) -> pd.DataFrame:
    '''
