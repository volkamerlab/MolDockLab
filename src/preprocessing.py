...
def merge_activity_values(
    ...
    """
    df_rescored = pd.read_csv(str(norm_scored_path))
    ...
    df_rescored[["id", "docking_tool", "pose"]] = df_rescored[scored_id_col].str.split(
        "_", expand=True
    )
    true_values_df = PandasTools.LoadSDF(str(mols_true_value_path)).rename(
        columns={true_value_idcol: "id"}
    )
    df_merged = df_rescored.merge(
        true_values_df[["id", true_value_col, activity_col]], on="id", how="left"
    )
    ...
    df_merged.to_csv(
        str(norm_scored_path.parent / "all_rescoring_results_merged.csv"), index=False
    )
    return df_merged
...
def _get_cluster_labels(scaffold_fps: list, min_cluster_size: int = 2) -> np.ndarray:
    ...
    clusterer = hdbscan.HDBSCAN(
        min_cluster_size=min_cluster_size, metric="jaccard"
    ).fit(scaffold_fps)
    return clusterer.labels_


def hdbscan_scaffold_split(
    original_data_path: Path, min_cluster_size: int
) -> pd.DataFrame:
    """
    ...
    cluster_labels = _get_cluster_labels(list(df["scaffold_fp"]), min_cluster_size)
...
def cv_split(
    ...
    train_test_ind = split_cv.split(
        clustered_df, groups=np.array(clustered_df["hdbscan_scaffold_cluster"])
    )
    ...
    for i, (train_idx, test_idx) in enumerate(train_test_ind):
        ...
        print(f"Train dataset has {len(train_id)} and test dataset has {len(test_id)}")
        ...
        train_df_norm.to_csv(
            str(output_path / f"{target_name}_train_{i}.csv"), index=False
        )
        test_df_norm.to_csv(
            str(output_path / f"{target_name}_test_{i}.csv"), index=False
        )


def plants_preprocessing(
    protein_file: Path, molecules_library: Path, ref_file: Path
) -> tuple:
    """
    ...
    for file in [protein_file, molecules_library, ref_file]:
        ...
        if converted_file.name not in os.listdir(converted_file.parent):
            obabel_command = (
                f"obabel -i{str(file)[-3:]} {str(file)} -O {str(converted_file)}"
            )
...
def norm_scores(
    ...
    for col in inversed_sf_cols:
        ...
        if col == "HYDE":
            ...
            df_copy.loc[:, col] = (
                df_copy.loc[:, col].apply(lambda x: min(x, 10000)) + hyde_ranks
            )
    ...
    scaled_numeric_df = pd.DataFrame(
        scaled_numeric, columns=numeric_cols, index=df_copy.index
    )
