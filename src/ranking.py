...
def runtime_cost_calculation(
    docking_tools: list, scoring_functions: list, num_poses=10
) -> float:
    """
...
def enrichment_factor_calc(
    df: pd.DataFrame, percent: int = 1, activity_class: str = "activity_class"
) -> float:
    """
    ...
    for i, comb in enumerate(splitted_comb):
            ...
            if ranking_method.startswith("weighted_ecr"):
                df_rank = weighted_ECR(
                    ...
                    mapped_weights=mapped_weights[
                        float(ranking_method_name.split("_")[-1])
                    ],
        ...
        try:
            ...
            df_unique_sorted = df_rank_copy.sort_values(
                by=df_rank_copy.columns[1], ascending=False
        ...
        try:
            ...
            ef = enrichment_factor_calc(
                df_unique_sorted, percent=10, activity_class="activity_class"
            )
...
def poses_ranking(
    ...
    df_rescored[["ID", "docking_tool", "pose"]] = df_rescored["ID"].str.split(
        "_", expand=True
    )
    ...
    rescoring_methods = [
        ...
        if col
        not in ["ID", "id", "pose", "docking_tool", "true_value", "activity_class"]
    ]
    ...
    if "weighted_ecr" in ranking_methods:
        ...
        ranking_methods.extend(
            [f"weighted_ecr_{alpha}" for alpha in mapped_weights.keys()]
        )
