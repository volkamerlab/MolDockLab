...
def _plants_pocket_generation(protein_file_mol2: Path, ref_file_mol2: Path):
    ...
    """
    plants_pocket_cmd = (
        f"./software/PLANTS --mode bind {str(ref_file_mol2)} {str(protein_file_mol2)}"
    )
...
def extract_binding_pocket(protein_file: Path, output_file: Path):
    ...
    if binding_pocket.exists():
        ...
        print(
            "Binding pocket file does not exist. Please provide a binding pocket file"
        )
        return
    ...
    for i in range(thread_num):
        ...
        if "chemplp" == scoring_function:
            ...
            if not (split_folder / f"sdf_{i}.mol2").exists():
                run_command(
                    f"obabel -isdf {split_folder / f'sdf_{i}.sdf'}"
            ...
            if not (split_folder / f"sdf_{i}.pdbqt").exists():
                run_command(
                    f"obabel {split_folder / f'sdf_{i}.sdf'}"
...
def read_posebusters_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df[["original_id", "docking_tool", "pose"]] = df["molecule"].str.split(
        "_", expand=True
    )
    ...
    df_grouped = df.groupby("docking_tool").apply(
        lambda x: x.shape[0] - x.select_dtypes(include=["bool"]).astype(int).sum()
    )

    unique_id_count = (
        df.groupby("docking_tool")["original_id"]
        .nunique()
        .reset_index(name="unique_id_count")
    )

    df_grouped["Number of Docked Molecules"] = list(unique_id_count["unique_id_count"])
...
def check_correlation_pairs(
    correlation_matrix: pd.DataFrame, threshold: float = 0.9
) -> list[tuple]:
    """
...
def handling_multicollinearity(
    df: pd.DataFrame, threshold: float = 0.9, true_value_col: str = "true_value"
) -> pd.DataFrame:
    """
    ...
    print(
        f"Scores of {columns_to_remove} were found to highly correlate. Therefore, they are removed."
    )
